...
func TestListNode_Add(t *testing.T) {
	...
	val, err = list.Get(list.listElements.Size() - 1)
	if val != 11 || err != nil {
		t.Errorf("expected 10, got %v", val)
	}
	...
	err = list.Add(13, list.listElements.Size()+1) // Add 12 at invalid index
	if err == nil {
		// Expect a error
		t.Errorf(err.Error())
	}
...
func TestListNode_AddAll(t *testing.T) {
	...
	for i := 0; i < list.listElements.Size(); i++ {
		val, err := list.Get(i)
		if val != i || err != nil {
			t.Errorf("expected %v, got %v", i, val)
		}
...
func TestListNode_Clear(t *testing.T) {
	...
	list.Clear()
	if list.Size() != 0 {
		t.Errorf("Expected new list size to be 0, got %v", list.Size())
	}
}
